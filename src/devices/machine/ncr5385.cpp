...
// internal flag *BEFORE* the IRQ3 generated by XFI_OUT_ACK is received.  Without this delay, the IRQ3 arrives
// before the setup has been executed and IRQ3 handler fails to process the interrupt. => HANGS
//
// A correctly function call sequence looks like this:
//
// [0x00012584: write block loops on:]
//    check AUX_STATUS_DATA_FULL, writes to SCSI_DAT
//  loop finishes
// [~10us for bra,bsr,movem.l,.. to arrive at:]
// [0x000127d2: SCSI_wait]
// 	  scsi_Status_flags = 1
//    scsi_aux_Status = 0
//    scsi_int_Status = 0
//														[NCR5385 raises IRQ3]
...
//
//
// A broken function call sequence looks like this:
//
// [write block loops on:]
//    check AUX_STATUS_DATA_FULL, writes to SCSI_DAT
// loop finishes
//														[NCR5385 raises IRQ3]
// ....
...
// Patching the IRQ_3 handler to NOT check scsi_Status_flags is set and make
// it always branch to SCSI_wait_continue avoids the race condition.
// 0x0001285e = 0x60 bra (was 0x66 bne)
//
// In order to boot at all (and apply the IRQ_3 handler patch) means we need a delay
// between AUX_STATUS_TC_ZERO and raising the IRQ3 of >10us
//
// 32us
unsigned constexpr TEK4404_XFI_OUT_ACK_DELAY = 32'000;
...
int ncr5385_device::state_step()
{
	...
	case XFI_OUT_REQ:
		if (ctrl & S_REQ)
		{
			...
			{
				// TEK4404_XFI_OUT_ACK_DELAY delay is to allow CPU to prep for IRQ_3
				// however, if there is a simultaenous IRQ_6, this delay is not long enough and
				// the IRQ_3 is run back-to-back with the IRQ6 not allowing CPU prep to run..=> HANG
				
				// TODO: could we know if there is presently an IRQ6 at this point?
